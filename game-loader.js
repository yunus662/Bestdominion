...
// A simple PlayerStatsManager for updating UI statistics
...
document.addEventListener("DOMContentLoaded", () => {
  (async function() {
    ...
    // --- Create a country selection overlay (UI mimic from your start page)
    function showCountrySelectionOverlay(features) {
      ...
      overlay.innerHTML = `
        ...
        <select id="country-select" style="padding: 10px; font-size: 1.2em; margin: 10px;">
          ${features.map(f => `<option value="${f.properties.ADMIN}">${f.properties.ADMIN}</option>`).join("")}
        </select>
    ...
    // Wait for the player to select a nation then remove the overlay.
    const playerNation = await new Promise(resolve => {
      document.getElementById("start-game-btn").addEventListener("click", () => {
        const selected = document.getElementById("country-select").value;
        console.log("Player selected nation:", selected);
        resolve(selected);
      });
    });
    overlay.remove();

    // --- Build a game menu overlay (displaying summary stats; UI styled similar to your start page)
    ...
    // --- Initialize the country system with the chosen nation
    initCountrySystem(map);

    // --- Create player units for the selected nation (instead of a hard-coded nation)
    const units = [
    ...
    // --- Process city data: add markers (with tooltips) from enriched city data
    ...
    // --- Setup fog-of-war & periodic reveal if units exist
    ...
    // --- Setup unit click handling with audio feedback.
    ...
    // --- Map click handling to move the selected unit.
    map.on("click", (e) => {
      if (selectedUnit) {
    ...
    // --- Log doctrine and government details using imported modules.
    ...
    // --- Optional: call AI initialization (uncomment if needed)
    // allUnitsAI(map);

    // --- Start the game clock so that UI stats update in real-time.
    ...
    console.log("game-loader.js: Gameplay initialized successfully.");
  })();
});
